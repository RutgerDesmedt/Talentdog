...
DB_PATH = os.getenv("DB_PATH", "database/talentdog.db")
...
async def get_smart_signals(company: str):
    if not SERPER_API_KEY: raise HTTPException(status_code=500, detail="Serper Key Missing")
    ...
    for sig_id, config in SIGNAL_CONFIG.items():
        if should_check(company, sig_id):
            ...
            # GEEN API CALL (Gratis uit database)
            current_data = get_cached_data(company, sig_id) or []

        if current_data:
            final_report.append({"signal_id": sig_id, "signal_label": sig_id.capitalize(), "found_items": current_data})
